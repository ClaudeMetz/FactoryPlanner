...
local function adjust_for_item_type(flow_product_bar, modal_data)
    ...
    if item_proto ~= nil then
        ...
        if item_proto.type == "fluid" then
            ...
            product_dialog.handle_belt_change(player, "")
        end
    end
end

-- Focus the textfield that this product is currently defined_by
local function set_appropriate_amount_focus(flow_product_bar, modal_data)
...
local function update_product_amounts(flow_product_bar, modal_data)
    local lob = modal_data.lanes_or_belts
...
function product_dialog.condition_instructions()
    return {
        data = {
