...
local function add_module_line(parent_flow, ui_elements, module, empty_slots, parent)
    parent_flow.add{type="label", caption={"fp.pu_module", 1}}
    ...
    local textfield_slider = parent_flow.add{type="textfield", name="fp_textfield_module_amount", text=slider_value}
...
function module_dialog.open(_, modal_data)
    ...
    local flow_module = ui_elements.content_frame.add{type="flow", direction="horizontal"}
    ...
    add_module_line(flow_module, ui_elements, module, empty_slots, machine)
...
function module_dialog.close(player, action)
    ...
    if action == "submit" then
        ...
        local new_module = Module.init_by_proto(module_proto, module_amount)
