...
function simplex_engine.solve_floor(floor_data, line_metadata_table, level, previous_basis, cache_invalid_map)
    ...
    for item_key, _ in pairs(products) do
        if not intermediates[item_key] then
            ...
            tableau:add_item_variable(item_key, floor_data.id, "export", objective)
        end
    end
    ...
    if level == 1 then
        ...
        for _, item in pairs(floor_data.products) do  ---@cast item SolverItem
            ...
            tableau:add_item_variable(item_key, floor_data.id, "desired_export", objective)
            tableau:add_item_constraint(item_key, floor_data.id, "desired_export", "<=", item.amount, objective)
        end
        ...
        for _, item in pairs({}) do  ---@cast item SolverItem
            ...
            tableau:add_item_variable(item_key, floor_data.id, "desired_import", objective)
            tableau:add_item_constraint(item_key, floor_data.id, "desired_import", "<=", item.amount, objective)
        end
