...
function _topological_sort.sort_floor(floor)
    ...
    local line_node = {}  ---@type table<ObjectID, SortNode>
    for line_object in floor:iterator() do
        line_node[line_object.id] = { products = {}, ingredients = {} }  ---@type SortNode
        if line_object.class == "Floor" then
            ...
            for _, item in pairs(line_object.products) do
                ...
                line_node[line_object.id].products[item_key] = product_index
            ...
            for _, item in pairs(line_object.ingredients) do
                local item_key = solver.util.pack_item(item.proto.name, item.proto.type)
                line_node[line_object.id].ingredients[item_key] = ingredient_index
    ...
    local function depth_first_search(line_id, path)
        local node = line_node[line_id]
        ...
        elseif node.visited < iteration then
            ...
            local weakest_edge = {item_key = "", cost = 0}  ---@type SortEdge
            for i = #path, 1, -1 do
                local edge = path[i]
        ...
        for item_key, product_index in pairs(node.products) do
            for neighbor_id, neighbor_node in pairs(line_node) do
                if neighbor_id ~= line_id then
                    local ingredient_index = neighbor_node.ingredients[item_key]
                    if ingredient_index then
                        local edge = { item_key = item_key, cost = 0.0 }  ---@as SortEdge
                        ...
                        local cycling_edge = depth_first_search(neighbor_id, path)

                        -- In the case of cycling, break the cycle on the selected edge
                        if cycling_edge and cycling_edge ~= table.remove(path) then
                            node.visited = nil
                            return cycling_edge
                        end
                    end
        ...
        table.insert(order, line_id)
    end

    for line_id, node in pairs(line_node) do
        if not node.visited then
            depth_first_search(line_id, {})
