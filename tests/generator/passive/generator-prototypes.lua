...
-- Sets up prototypes covering every power generating machine configuration.
-- Inert until called; wire it up via `require("generator-prototypes")()` from data.lua,
-- or split the sections into individual suite setups once checks exist for them.
-- Everything it needs is defined below, so it can be loaded on its own or alongside the
-- other passive files, whose prototypes it deliberately doesn't share.
--
...
-- category and the fuel list per machine are the interesting part of each case.
--
