...
-- Covers every fluid energy source configuration, old and new: setup defines the
-- prototypes, check asserts what the generator made of them.
--
-- Coverage:
...
return {
    setup = function()
        ...
        -- Has heat capacity, but no room above its default temperature, so it can
        -- never carry heat and must not be offered as a heat fuel. The maximum is
        -- removed explicitly: with one, the boiler case's unfiltered heat-in-place
        -- boiler would produce a hot variant when both load into the same world
        local cold_fluid = deepcopy(data.raw.fluid.water)
        cold_fluid.name = "test-cold-fluid"
        cold_fluid.heat_capacity = "1kJ"
        cold_fluid.max_temperature = nil
    ...
    check = function()
        ...
        local function machine(name, fuel_category, burner)
            return helpers.check_machine(c, name, {category = "test-fluid-powered",
                fuel_category = fuel_category, burner = burner})
        end

        local burn_open = machine("test-machine-burn-open", "fluid-fuel",
            {burns_fluid = true, scale_fluid_usage = true})
        c.check(burn_open and burn_open.burner.fluid_usage_per_tick == nil,
            "burn-open: expected no usage cap of its own")
        machine("test-machine-burn-filtered", "fluid-fuel-filter-test-oil",
            {burns_fluid = true, fluid_usage_per_tick = 0.5})
        machine("test-machine-heat-capped", "fluid-heat-filter-test-heat-fluid",
            {burns_fluid = false, scale_fluid_usage = true, fluid_usage_per_tick = 0.005})

        -- With no usage set, the game derives it from the source's maximum_temperature
        local derived = machine("test-machine-heat-derived", "fluid-heat-filter-test-heat-fluid",
            {scale_fluid_usage = false})
        c.check(derived and helpers.approx(derived.burner.fluid_usage_per_tick, 1250 / 142500),
            "heat-derived: expected usage derived from maximum_temperature")

        -- Box bounds bind with and without a filter, sharing categories either way
        machine("test-machine-heat-open", "fluid-heat")
        machine("test-machine-heat-bounded", "fluid-heat-filter-test-heat-fluid",
            {minimum_temperature = 300, maximum_temperature = 500})
        machine("test-machine-heat-bounded-open", "fluid-heat",
            {minimum_temperature = 300})

        -- The solid burner next to them must be entirely untouched by fluid handling
        local solid = machine("test-machine-solid", "chemical")
        c.check(solid and solid.burner.burns_fluid == nil,
            "solid burner: expected no fluid fields on its burner")

        -- Filtered categories only fit their fluid; unfiltered ones offer all valid options
        local filtered = find("fuels", nil, "fluid-fuel-filter-test-oil")
        c.check(filtered and table_size(filtered.members) == 1,
            "fluid-fuel-filter-test-oil: expected exactly one fuel")
        c.check(find("fuels", "test-oil", "fluid-fuel") ~= nil,
            "test-oil: missing from the unfiltered category")
        local diesel = find("fuels", "test-diesel", "fluid-fuel")
        c.check(diesel and helpers.approx(diesel.fuel_value, 3e6),
            "test-diesel: expected burnable fuel worth 3MJ")

        c.check(find("fuels", "test-heat-fluid", "fluid-heat") ~= nil,
            "test-heat-fluid: missing as a heat fuel")
        c.check(find("fuels", "steam", "fluid-heat") ~= nil,
            "steam: missing as a heat fuel")
        c.check(find("fuels", "test-cold-fluid", "fluid-heat") == nil,
            "test-cold-fluid: never produced hot, must not be a heat fuel")
        c.check(find("fuels", "test-hidden-fluid", "fluid-heat") == nil,
            "test-hidden-fluid: hidden, must not be a heat fuel")
